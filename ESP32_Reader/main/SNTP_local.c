...
{
    char *p;
    p = strtok(strftime_buf, " ");
    p = strtok(NULL, " ");
    p = strtok(NULL, " ");
    p = strtok(NULL, " ");
    p = strtok(p, ":");

    sscanf(p, "%d", &_now._Hour);
    p = strtok(NULL, ":");
    sscanf(p, "%d", &_now._Min);
    p = strtok(NULL, ":");
    sscanf(p, "%d", &_now._Sec);
}
...
void task_SNTP(void)
{
    ...
    ESP_LOGI(TAG_SNTP, "The current date/time in VietNam is: %s", strftime_buf);
